...
public static class Hashing
{
    ...
    public static CodeFragment Generate(TypeShape shape)
    {
        ...
        typeBody.AppendLine($"        /// <summary>Returns a hash code for the <see cref=\"{shape.TypeName}\" />.</summary>");
        typeBody.AppendLine("        [MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]");
    ...
    private sealed class PrimeCounter
    {
        private uint _index;
    ...
    private static void EmitHash(TypeShape shape, StringBuilder body, bool wide, PrimeCounter counter)
    {
        ...
        if (shape.BaseType == BaseType.Bool)
        {
            ...
            for (var col = 0; col < shape.Columns; col++)
            {
                ...
                body.Append($"select({FmtPrimes(shape.Rows, counter)}, {FmtPrimes(shape.Rows, counter)}, {colName})");
            }
            body.AppendLine(");");
        }
        else
