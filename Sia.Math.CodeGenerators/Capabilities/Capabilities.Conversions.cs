...
{
    private static readonly string[] Imports = ["System.Runtime.CompilerServices", "System.Runtime.Intrinsics"];
    ...
    public static CodeFragment Generate(TypeShape shape)
    {
        ...
        GenerateConversion(shape, shape.BaseType, false, true, typeBody, mathBody);

        foreach (var srcType in GetSourceTypes(shape.BaseType)) {
            var isExplicit = NeedsExplicit(shape.BaseType, srcType);
            GenerateConversion(shape, srcType, isExplicit, true, typeBody, mathBody);
            if (!shape.IsScalar) {
                GenerateConversion(shape, srcType, isExplicit, false, typeBody, mathBody);
            }
        }
    ...
    private static void GenerateConversion(TypeShape shape, BaseType srcType, bool isExplicit, bool isScalar,
